...
import { Plus, Globe, Lock, Copy, Trash2, LogOut, Settings, Users } from 'lucide-react';
...
export default function Dashboard() {
  ...
  async function createApp() {
    ...
    const { data, error } = await supabase
      .from('apps')
      .insert({ owner_id: session.user.id, name: 'Nieuwe App', ngc_code: DEFAULT_NGC_CODE })
      .select()
      .single();
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={createApp}
            disabled={creating}
            className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium transition-colors bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
          >
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-border p-12 text-center" style={{ background: 'hsl(var(--card))' }}>
            ...
            <button
              onClick={createApp}
              disabled={creating}
              className="inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90"
            >
              <Plus className="h-4 w-4" />
              Nieuwe App
            </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {myApps.map(app => (
              <div
                key={app.id}
                className="group rounded-xl border border-border p-4 transition-all hover:border-primary/40 cursor-pointer"
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Laatst bewerkt: {new Date(app.updated_at).toLocaleDateString('nl-NL')}
                </p>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                  <button
                    onClick={() => togglePublic(app)}
                    className="rounded p-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors"
                    title={app.is_public ? 'Maak privé' : 'Maak publiek'}
                  >
                    {app.is_public ? <Lock className="h-3.5 w-3.5" /> : <Globe className="h-3.5 w-3.5" />}
                  </button>
                  <button
                    onClick={() => toggleRemixable(app)}
                    className="rounded p-1.5 text-xs text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors"
                    title={app.is_remixable ? 'Remix uit' : 'Remix aan'}
                  >
                    <Copy className="h-3.5 w-3.5" />
                  </button>
                  <button
                    onClick={() => deleteApp(app.id, app.name)}
                    className="rounded p-1.5 text-xs text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors ml-auto"
                    title="Verwijderen"
                  >
          ...
          <div className="mt-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {sharedApps.map(app => (
                <div
                  key={app.id}
                  className="rounded-xl border border-border p-4 transition-all hover:border-accent/40 cursor-pointer"
                  style={{ background: 'hsl(var(--card))' }}
                  onClick={() => navigate(`/editor/${app.id}`)}
                >
